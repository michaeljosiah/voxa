...
- **Background agent delegation (talker/thinker split)** — speced as **VDX-008** ([spec](docs/specifications/vdx-008-background-agent-spec.html)): a fast interaction model keeps the voice-latency budget while a heavyweight second `IAgentTurnDriver` runs tools/browsing off the critical path; explicit `delegate_task` opt-in (no mirroring), frontend-tool-style frame round-trip (`BackgroundTaskRequestFrame` down / `BackgroundTaskCompletedFrame` up), results re-enter as agent-initiated turns gated for relevance by the interaction model. Composer keyed-driver seam; **byte-identical when unused**.
