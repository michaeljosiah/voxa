        ...
        foreach (var node in chain)
        {
            switch (node.Kind)
            {
                ...
                case BuilderNodeKind.Vad:
                    ...
                    sb.AppendLine("        SampleRate:          16000, // match the STT provider's input rate");
                ...
                case BuilderNodeKind.Stt:
                    sb.AppendLine();
                    sb.AppendLine($"    // {node.Provider} STT (options from the Voxa:{node.Provider} config section)");
                    sb.AppendLine($"    if (!registry.TryGetStt(\"{node.Provider}\", out var stt))");
                    sb.AppendLine($"        throw new InvalidOperationException(\"STT '{node.Provider}' is not registered.\");");
                ...
                case BuilderNodeKind.Agent:
                    ...
                    sb.AppendLine("    var agent = session.GetRequiredService<IVoiceAgentFactory>().Create(session, options.Agent)");
                    sb.AppendLine("        ?? throw new InvalidOperationException(\"Voxa:Agent:Provider is not set.\");");
                    sb.AppendLine("    builder.Then(MicrosoftAgentVoice.CreateProcessor(agent));");
                    break;
