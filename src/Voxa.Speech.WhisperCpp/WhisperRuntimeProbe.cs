...
public static class WhisperRuntimeProbe
{
    ...
    /// </summary>
    public static bool IsAvailable(WhisperDevice device) => device switch
    {
        WhisperDevice.Cpu or WhisperDevice.Auto => true,
        WhisperDevice.Cuda => BackendDeployed("cuda"),
        WhisperDevice.Vulkan => BackendDeployed("vulkan"),
        WhisperDevice.CoreML => BackendDeployed("coreml"),
        _ => false,
    };
    ...
    private static bool BackendDeployed(string backend)
    {
        if (CurrentRid() is not { } rid) return false;
        return Directory.Exists(Path.Combine(AppContext.BaseDirectory, "runtimes", backend, rid));
    }
